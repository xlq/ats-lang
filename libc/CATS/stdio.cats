...
ats_int_type atslib_fseek_exn
  ...
  if (err < 0) {
    perror ("fseek") ; ats_exit_errmsg (1, "Exit: [fseek] failed\n") ;
  }
  return ;
} /* atslib_fseek_exn */
...
atslib_ftell_exn
  ...
  if (ret < 0) {
    ats_exit_errmsg (1, "Exit: [ftell] failed\n") ;
  }
  return ret ;
}
...
atslib_tmpfile_exn () {
  FILE* fil =  tmpfile() ;
  if (!fil) {
    ats_exit_errmsg (1, "Exit: [tmpfile] failed\n") ;
  }
  return fil ;
} /* end of [atslib_tmpfile] */
