...
#include "LETPAR/quicksort_mt.dats"
...
in
  if n > CUTOFF then let
    ...
    val () = workshop_insert_work (ws, f) where {
      val f = lam {l:addr} (ws: !WSptr l)
        : void =<lincloptr1> quicksort_splt (ws, A, i_pivot)
      val f = work_encode (f)
    } // end of [val]
    val () = workshop_insert_work (ws, f) where {
      val f = lam {l:addr} (ws: !WSptr l)
        : void =<lincloptr1> quicksort_splt (ws, A+ofs+sizeof<T>, n-i_pivot-1)
      val f = work_encode (f)
    } // end of [val]
  ...
  in
    ...
    val () = quicksort (pf | A, n)
    prval () = fpf (pf)
  in
    // empty
  end (* end of [if] *)
end // end of [quicksort_splt]
...
//
  ...
  val t = quicksort_splt (ws, A, n)
//
  val () = workshop_wait_worker_blocked (ws)
  // val () = (array_ptr_print (pfarr | A, nsz); print_newline ())
