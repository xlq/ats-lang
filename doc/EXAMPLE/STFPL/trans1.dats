...
staload M = "FUNMAP/funmap_avltree.dats"
...
assume theConstTypMap_t = $M.map_t (sym, t1yp)
...
val theConstTypMap = loop (cts, ini) where {
  ...
  fun loop (cts: List T, res: theConstTypMap_t): theConstTypMap_t =
    case+ cts of
    | ct :: cts => let
        val res = $M.funmap_insert (res, ct.0, ct.1, cmp) in loop (cts, res)
      end // end of [::]
    | nil () => res
  // end of [loop]  
} // end of [val]
...
implement theConstTypFind (opr) = let
  val+ OPR sym = opr
  val ans = $M.funmap_search<sym,t1yp> (theConstTypMap, sym, cmp)
in
  case+ ans of
  | ~Some_vt typ => typ
  | ~None_vt () => let
...
typedef ctx = $M.map_t (sym, v1ar)

fun ctx_extend_arglst
  (G: ctx, args: a0rglst): @(ctx, v1arlst) = @(G, xs) where {
  var xs: List_vt v1ar = list_vt_nil ()
  fun loop
    (G: ctx, args: a0rglst, xs: &List_vt v1ar): ctx = case+ args of
    | list_cons (arg, args) => let
        ...
        val G = $M.funmap_insert (G, sym, x, cmp_sym_sym)
      in
        loop (G, args, xs)
      end // end of [list_cons]  
    | list_nil () => G
  // end of [loop]
  val G = loop (G, args, xs)
...
implement trans1_exp (e) = auxExp (G0, e) where {
  #define cmp cmp_sym_sym
  val G0 = $M.funmap_empty ()
...
*)
  ...
  in
    ...
    | E0XPfix (sym, args, oty, body) => let
        val (G, xs) = ctx_extend_arglst (G, args)
        val ts_arg =
    ...
    | E0XPvar sym => let
        val ans = $M.funmap_search (G, sym, cmp) in
        case+ ans of
        | ~Some_vt x => e1xp_make_var (loc0, x)
        | ~None_vt _ => abort {e1xp} (1) where {
            val () = $POSLOC.prerr_location loc0
            val () = prerr ": exit(STFPL)"
...
//
  and auxDec (G: ctx, dec: d0ec): @(ctx, d1ec) =
    case+ dec.d0ec_node of
    | D0ECval (isrec, vds) => let
        val (G, vds) = auxValdeclst (G, isrec, vds)
...
//
  and auxValdeclst
    (G: ctx, isrec: bool, vds: v0aldeclst): (ctx, v1aldeclst) =
    case+ 0 of
    | _ when isrec => let
        ...
        val G = loop (G, xs) where {
          fun loop (G: ctx, xs: v1arlst): ctx = case+ xs of
            | list_cons (x, xs) => let
                val G = $M.funmap_insert (G, x.v1ar_nam, x, cmp_sym_sym)
              in
                loop (G, xs)
              end // end of [list_cons]
            | list_nil () => G
    ...
    | _ (*nonrec*) => (G, vds) where {
        ...
        val (G, vds) = aux (G, vds, defs) where {
          fun aux {n:nat} .<n>. (
              G: ctx
            , vds: list (v0aldec, n)
            , defs: list (e1xp, n)
            ) : (ctx, list (v1aldec, n)) = case+ vds of
            | list_cons (vd, vds) => let
  ...
  (* end of [auxDec] *)
//
  and auxDeclst (G: ctx, decs: d0eclst): @(ctx, d1eclst) =
    case+ decs of
    | list_cons (dec, decs) => let
        val (G, dec) = auxDec (G, dec)
        val (G, decs) =  auxDeclst (G, decs)
      in
        (G, list_cons (dec, decs))
      end // end of [list_cons]
    | list_nil () => (G, list_nil ())
  // end of [auxDeclst]  
} (* end of [trans1_exp] *)
