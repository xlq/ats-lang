...
**
** the code is a direct translation from the C submission by
...
fn pidigits (
  ...
  var d: uint with pf_d = 0U
  var k: uint with pf_k = 0U
  var i: int = 0 and m: int = 0
  val () = while (true) let
    val () = loop
      (pf_tmp, pf_acc, pf_num, pf_den, pf_k, pf_d | (*none*)) where {
      fun loop (
          pf_tmp: !v_tmp, pf_acc: !v_acc, pf_num: !v_num, pf_den: !v_den
      ...
      in
        if sgn > 0 then
          loop (pf_tmp, pf_acc, pf_num, pf_den, pf_k, pf_d | (*none*))
        else let
          val () =
