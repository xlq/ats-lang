...
implement d2sym_make (loc, q, id, d2is) = '{
...
implement s2varlstord_add (s2vs0, s2v0) = begin
  case+ s2vs0 of
  | cons (s2v, s2vs) => begin
...
implement p2atlst_svs_union (p2ts) = let
...
implement p2atlst_dvs_union (p2ts) = let
...
implement labp2atlst_svs_union (lp2ts) = let
...
implement labp2atlst_dvs_union (lp2ts) = let
...
implement p2at_as (loc, refknd, d2v, p2t) = let
...
implement p2at_con (loc, refknd, d2c, qua, res, npf, p2ts_arg) = let
...
implement p2at_exist (loc, s2vs, p2t) = '{
  p2at_loc= loc
...
implement p2at_list (loc, npf, p2ts) = let
...
implement p2at_lst (loc, p2ts) = let
  val svs = p2atlst_svs_union (p2ts)
  val dvs = p2atlst_dvs_union (p2ts)
in '{
  p2at_loc= loc
...
implement p2at_rec (loc, recknd, npf, lp2ts) = let
...
implement p2at_tup (loc, tupknd, npf, p2ts) = let
  val svs = p2atlst_svs_union (p2ts)
  val dvs = p2atlst_dvs_union (p2ts)
...
implement p2at_var (loc, refknd, d2v) = '{
  p2at_loc= loc
, p2at_svs= s2varlstord_nil
...
implement d2exp_ann_type (loc, d2e, s2e) = '{
  d2exp_loc= loc
, d2exp_node= D2Eann_type (d2e, s2e)
, d2exp_typ= None ()
}

implement d2exp_ann_seff (loc, d2e, s2fe) = '{
  d2exp_loc= loc
, d2exp_node= D2Eann_seff (d2e, s2fe)
, d2exp_typ= None ()
}

implement d2exp_ann_funclo (loc, d2e, fc) = '{
  d2exp_loc= loc
, d2exp_node= D2Eann_funclo (d2e, fc)
, d2exp_typ= None ()
}

implement d2exp_apps (loc, d2e_fun, d2as_arg) = '{
  d2exp_loc= loc
...
implement d2exp_app_sta_dyn
  (loc0, loc1, d2e_fun, sarg, loc_arg, npf, darg) = let
...
implement d2exp_floatsp (loc, f(*string*)) = '{
  d2exp_loc= loc, d2exp_node= D2Efloatsp (f), d2exp_typ= None ()
}

implement d2exp_foldat (loc, s2as, d2e) = '{
  d2exp_loc= loc
, d2exp_node = D2Efoldat (s2as, d2e)
, d2exp_typ= None ()
}

implement d2exp_for (loc, inv, ini, test, post, body) = '{
  d2exp_loc= loc
, d2exp_node= D2Efor (inv, ini, test, post, body)
, d2exp_typ= None ()
}

implement d2exp_freeat (loc, s2as, d2e) = '{
  d2exp_loc= loc
...
implement d2exp_if
  (loc, inv, d2e_cond, d2e_then, od2e_else) = '{
  d2exp_loc= loc
, d2exp_node= D2Eif (inv, d2e_cond, d2e_then, od2e_else)
, d2exp_typ= None ()
} // end of [d2exp_if]

implement d2exp_int (loc, str, int) = '{
  d2exp_loc= loc
, d2exp_node= D2Eint (str, int)
, d2exp_typ= None ()
}

implement d2exp_intsp (loc, str, int) = '{
  d2exp_loc= loc
...
implement d2exp_sym (loc, d2s) = '{
  d2exp_loc= loc, d2exp_node= D2Esym (d2s), d2exp_typ= None ()
}

implement d2exp_tmpid (loc, d2e, ts2ess) = '{
  d2exp_loc= loc
, d2exp_node= D2Etmpid (d2e, ts2ess)
, d2exp_typ= None ()
}

implement d2exp_top (loc) = '{
...
implement d2lab_lab (loc, lab) = '{
  d2lab_loc= loc, d2lab_node= D2LABlab lab
}

implement d2lab_ind (loc, ind) = '{
  d2lab_loc= loc, d2lab_node= D2LABind ind
}

(* ****** ****** *)

implement i2nvarg_make (d2v, os2e) =
  '{ i2nvarg_var= d2v, i2nvarg_typ= os2e }

implement i2nvresstate_nil = '{
...
implement i2nvresstate_make (s2vs, s2ps, arg) = '{
...
implement i2nvresstate_make_met (s2vs, s2ps, arg, met) = '{
  i2nvresstate_svs= s2vs
, i2nvresstate_gua= s2ps
, i2nvresstate_arg= arg
...
implement loopi2nv_make (loc, s2vs, s2ps, met, arg, res) = '{
  loopi2nv_loc= loc
, loopi2nv_svs= s2vs
, loopi2nv_gua= s2ps
, loopi2nv_met= met
...
implement i2nvresstate_update (res) = let
...
implement loopi2nv_update (i2nv) = let
...
implement m2atch_make (loc, d2e, op2t) = '{
  m2atch_loc= loc, m2atch_exp= d2e, m2atch_pat= op2t
} // end of [m2atch_make]

implement c2lau_make (loc, p2t, gua, seq, neg, d2e) = '{
  c2lau_loc= loc
, c2lau_pat= p2t
, c2lau_gua= gua
, c2lau_seq= seq
...
implement s2tavar_make (loc, s2v) =
  '{ s2tavar_loc= loc, s2tavar_var = s2v }

implement s2aspdec_make (loc, s2c, def) = '{
...
implement d2ec_none (loc) = '{
  d2ec_loc= loc, d2ec_node= D2Cnone ()
}

implement d2ec_list (loc, d2cs) = '{
  d2ec_loc= loc, d2ec_node= D2Clist d2cs
}

implement d2ec_include (loc, d2cs) = '{
...
implement d2ec_saspdec (loc, d2c) = '{
