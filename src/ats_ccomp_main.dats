...
in
  ...
  fprintf (
    pf
  | out
  , " * The compilation time is: %i-%i-%i: %2ih:%2im\n *\n */\n\n"
...
fn emit_include_header {m:file_mode}
  (pf: file_mode_lte (m, w) | out: &FILE m): void = let
  val () = fprint (pf | out, "/* include some .h files */\n")
  val () = fprint (pf | out, "#include \"ats_basics.h\"\n")
  val () = fprint (pf | out, "#include \"ats_exception.h\"\n")
  val () = fprint (pf | out, "#include \"ats_memory.h\"\n")
  val () = fprint (pf | out, "#include \"ats_types.h\"\n")
  val () = fprint (pf | out, '\n')
in
  // empty
...
fn emit_include_cats {m:file_mode}
  ...
  val () = fprint (pf | out, "#include \"prelude/CATS/byte.cats\"\n")
  val () = fprint (pf | out, "#include \"prelude/CATS/char.cats\"\n")
  val () = fprint (pf | out, "#include \"prelude/CATS/float.cats\"\n")
  val () = fprint (pf | out, "#include \"prelude/CATS/integer.cats\"\n")
  val () = fprint (pf | out, "#include \"prelude/CATS/pointer.cats\"\n")
...
in
  if string1_is_at_end (name, 0) then false else eq_char_char (name[0], '\[')
...
fn fprint_atarray_name {m:file_mode}
  ...
  fun aux {n,i:nat | i <= n}
    ...
    : void = begin
    if string1_is_at_end (name, i) then ()
    else let
      val c = name[i]
    in
      if c <> ']' then begin
        fprint (pf | out, c); aux (out, name, i+1)
      end
    end // end of [if]
  end // end of [aux]
  val name = string1_of_string0 name
in
  if string1_is_at_end (name, 0) then () else aux (out, name, 1)
...
fn emit_typdef_sum {m:file_mode} (
  ...
  fun aux (out: &FILE m, i: int, names: strlst)
    ...
    | list_cons (name, names) => let
        val () = fprint (pf | out, name);
        val () = fprintf (pf | out, " atslab_%i", @(i))
        val () = fprint (pf | out, " ;\n")
      in
        aux (out, i+1, names)
...
fn emit_datcstlst {m:file_mode} (
  ...
  fun aux_conlst
    ...
    | D2CONLSTcons (d2c, d2cs) => let
        val () = fprint (pf | out, "ATSglobal(ats_sum_type, ")
...
fn emit_exnconlst {m:file_mode} (
  ...
  fun aux (
    ...
    | EXNCONLSTcons (d2c, !d2cs1) => let
        val () = fprint (pf | out, "ATSglobal(ats_exn_type, ")
        val () = emit_d2con (pf | out, d2c)
        val () = fprint (pf | out, ") ;\n")
...
fn emit_free_glocstlst {m:file_mode} (
  ...
  fun aux (out: &FILE m, i: int, xs: glocstlst): int = begin
    ...
    | ~GLOCSTLSTcons_clo (d2c, xs) => let
        val () = fprint (pf | out, "ATSglobal(ats_clo_ptr_type, ")
    ...
    | ~GLOCSTLSTcons_fun (d2c, xs) => let
        val () = fprint (pf | out, "ATSglobal(ats_fun_ptr_type, ")
        val () = emit_d2cst (pf | out, d2c)
        val () = fprint (pf | out, ") ;\n")
      in
        aux (out, i+1, xs)
...
fn _emit_dynconset {m:file_mode} {l:addr} (
  ...
  fn f_con (pf: !V | d2c: d2con_t, env: !VT): void = let
    ...
    val () = fprint (pf_mod | !p_l, "ATSextern(")
    val () = case+ 0 of
      | _ when d2con_is_exn d2c => begin
          fprint (pf_mod | !p_l, "ats_exn_type, ")
        end
      | _ => begin
          fprint (pf_mod | !p_l, "ats_sum_type, ")
        end
...
in
  ...
  | HITfun (fc, hits_arg, hit_res) => begin
    ...
    | $Syn.FUNCLOclo _ => let
      ...
      in
        fprint (pf | out, "ATSextern(ats_clo_ptr_type, ");
        emit_d2cst (pf | out, d2c);
        fprint (pf | out, ") ;\n");
        fprint (pf | out, "extern ");
    ...
    | $Syn.FUNCLOfun _ => let
      ...
      in
        ...
        | _ => begin
            fprint (pf | out, "ATSextern(ats_fun_ptr_type, ");
            emit_d2cst (pf | out, d2c);
            fprint (pf | out, ") ;\n");
            fprint (pf | out, "extern ");
            emit_hityp (pf | out, hit_res);
...
fn emit_extvallst_markroot {m:file_mode}
  ...
  fun aux (out: &FILE m, i: int, exts: !extvallst)
    : int = begin case+ exts of
    | EXTVALLSTcons (name, vp, !exts_rest) => let
        val () = fprint (pf | out, "ATS_GC_MARKROOT (&")
        val () = fprint (pf | out, name)
  ...
  fun aux (out: &FILE m, d2cs: d2eclst): void = begin case+ d2cs of
    | list_cons (d2c, d2cs) => let val () = case+ d2c.d2ec_node of
        | D2Cextcode (pos, code) => begin
            if (pos >= 0) then () else fprint_string (pf | out, code)
          end
...
fn emit_staload {m:file_mode} (
  ...
  fun aux_staload_dec (out: &FILE m, fils: !stafilelst)
    : void = begin case+ fils of
    | STAFILELSTcons (fil, !fils_rest) => let
        val () = fprint (pf | out, "extern ats_void_type")
        val () = fprint (pf | out, '\n')
        val () = emit_filename (pf | out, fil)
        val () = fprint (pf | out, "__staload (void) ;\n")
  ...
  fun aux_staload_datcstlst
    ...
    fun aux_conlst (out: &FILE m, d2cs: d2conlst)
      ...
      | D2CONLSTcons (d2c, d2cs) => let
          val () = begin
            if d2con_arity_real_get d2c > 0 then fprint (pf | out, "// ")
          end
          val () = emit_d2con (pf | out, d2c);
          val tag = d2con_tag_get d2c
          val () = fprintf (pf | out, ".tag = %i ;\n", @(tag))
        in
          aux_conlst (out, d2cs)
        end
      | D2CONLSTnil () => ()
  ...
  fun aux_staload_exnconlst
    ...
    fun aux (out: &FILE m, i: int, d2cs: !exnconlst): int = begin
      ...
      | EXNCONLSTcons (d2c, !d2cs1) => let
          ...
          val () = fprint (pf | out, ".name = \"")
(*
          val () = $Fil.fprint_filename (pf | out, d2con_fil_get d2c)
          val () = fprint (pf | out, "::")
...
*)
  ...
  val () = emit_filename (pf | out, fil)
  val () = fprint (pf | out, "__staload_flag = 0 ;\n\n")
  val () = fprint (pf | out, "ats_void_type ")
  val () = emit_filename (pf | out, fil)
  val () = fprint (pf | out, "__staload () {\n")
  val () = fprint (pf | out, "if (");
  val () = emit_filename (pf | out, fil)
  val () = fprint (pf | out, "__staload_flag) return ;\n")
  val () = emit_filename (pf | out, fil)
...
fn emit_dynload {m:file_mode} (
  ...
  val () = aux_dynload_dec (out, dynfils) where {
    fun aux_dynload_dec
      ...
      | DYNFILELSTcons (fil, !fils_rest) => let
          val () = fprint (pf | out, "int ")
          val () = emit_filename (pf | out, fil)
          val () = fprint (pf | out, "__dynload_flag = 0 ;\n")
          val () = fprint (pf | out, "extern ats_void_type\n")
          val () = emit_filename (pf | out, fil)
          val () = fprint (pf | out, "__dynload (void) ;\n")
  ...
  val () = let
    val () =
      if dynloadflag = 0 then fprint (pf | out, "// ")
    val () = emit_filename (pf | out, fil)
    val () = fprint (pf | out, "__dynload_flag = 1 ;\n")
  in
    // empty
  end // end of [val]
  ...
  in
    ...
    | _ when stropt_is_some name => let
        ...
        val () = fprintf (pf | out, "ats_void_type %s () {\n", @(name))
        val () = emit_filename (pf | out, fil)
        val () = fprint (pf | out, "__dynload () ; return ;\n}\n\n")
      in
        // empty
      end
    | _ => ()
...
fn emit_extypelst_free {m:file_mode}
  ...
  fun aux (out: &FILE m, i: int, ets: extypelst): int =
    ...
    | ~EXTYPELSTcons (name, hit_def, ets) => let
        ...
        val () = fprintf (pf | out, "typedef %s ", @(name_def))
...
implement ccomp_main {m}
  ...
  val () = // declaration for dynamic constants
    if (flag > 0) then let
      val () = fprint (pf | out, "/* external dynamic constant declarations */\n")
  ...
  val () = let
    val () = fprint (pf | out, "/* sum constructor declarations */\n")
  ...
  val () = let
    val () = fprint (pf | out, "/* exn constructor declarations */\n")
  ...
  val () = // declaring implemented constants that are not functions
    if (flag > 0) then let
      ...
      val () = begin
        fprint (pf | out, "/* global dynamic (non-functional) constant declarations */\n")
      end
  ...
  val () =
    if (flag > 0) then let
      val () = begin
        fprint (pf | out, "/* internal function declarations */\n")
      end
  ...
  val () = // static variable declarations
    if (flag > 0) then let
      val () = begin
        fprint (pf | out, "/* static temporary variable declarations */\n")
      end
  ...
  val () = // external variable declarations
    if (flag > 0) then let
      val () = begin
        fprint (pf | out, "/* external value variable declarations */\n\n")
      end
  ...
  val fls = (
    if (flag > 0) then let
      val () = fprint (pf | out, "/* function implementations */\n\n")
      val () = emit_funentry_lablst (pf | out, fls)
    in
      fls
    end else begin
  ...
  val () = let
    val () = fprint (pf | out, "/* static load function */\n\n")
  ...
  val () = begin case+ 0 of
    | _ when mainats_kind >= 0 => begin
        emit_mainfun (pf | out, fil); fprint (pf | out, '\n')
      end
    | _ => ()
  end // end of [val]
  ...
  val () = fprint (pf | out, "/* external typedefs */\n")
  val () = let
  ...
  val () = fprint (pf | out, "/* external codes at mid */\n")
  val n = emit_extcodelst (pf | out, 1(*mid*), extcodes)
  val () = if n > 0 then fprint (pf | out, '\n')
  val () = fprint (pf | out, "/* external codes at bot */\n")
