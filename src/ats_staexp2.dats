...
implement eq_s2rtdat_s2rtdat (s2td1, s2td2) = let
  val stamp1 = let val (vbox pf1 | p1) = s2td1 in p1->s2rtdat_stamp end
...
implement s2rt_fun (s2ts_arg, s2t_res) =  S2RTfun (s2ts_arg, s2t_res)
...
implement s2exp_app_srt (s2t, s2e_fun, s2es_arg) = '{
  s2exp_srt= s2t
, s2exp_node= S2Eapp (s2e_fun, s2es_arg)
}
...
implement s2exp_exi (s2vs, s2ps, s2e_scope) =
  case+ (s2vs, s2ps) of
  | (nil (), nil ()) => s2e_scope
  | (_, _) => '{
...
implement s2exp_lam_srt (s2t_fun, s2vs_arg, s2e_body) = '{
  s2exp_srt= s2t_fun, s2exp_node= S2Elam (s2vs_arg, s2e_body)
}

implement s2exp_metfn (d2vopt(*stamp*), met, s2e_fun) = '{
  s2exp_srt= s2e_fun.s2exp_srt, s2exp_node= S2Emetfn (d2vopt, met, s2e_fun)
}
...
implement s2exp_refarg (refval, s2e): s2exp = '{
...
implement s2exp_tylst (s2es: s2explst): s2exp = '{
  s2exp_srt= s2rt_types, s2exp_node= S2Etylst s2es
}

implement s2exp_tyrec_srt (s2t, k, npf, ls2es) = '{
  s2exp_srt= s2t, s2exp_node= S2Etyrec (k, npf, ls2es)
}

implement s2exp_uni (s2vs, s2ps, s2e_scope) =
  case+ (s2vs, s2ps) of
  | (nil (), nil ()) => s2e_scope
  | (_, _) => '{
      s2exp_srt= s2e_scope.s2exp_srt
